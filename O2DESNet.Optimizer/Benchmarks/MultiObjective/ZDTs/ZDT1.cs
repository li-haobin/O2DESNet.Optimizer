    ...
    public class ZDT1 : ZDTx
    {
        ...
        public override IList<double> Evaluate(IList<double> x)
        {
            if (!this.IsFeasible(x))
                return new List<double> { double.PositiveInfinity, double.PositiveInfinity };
            return new List<double> { Get_g1(x), Get_g2(x) };
        }
        ...
        {
            double f = 1.0;
            for (int i = 1; i < NDecisions; i++) f += 9 * x[i] / (NDecisions - 1);
            return f;
        }
        ...
        {
            var d = x.Select(v => 9.0 / (NDecisions - 1)).ToArray();
            d[0] = 0;
            return (DenseVector)d;
        }
